 ...
 * @return {void}
 */
YX.fn.buildTourists = function (id) {
	var data = {
		tourists: this.cache.getTeamlist()
	}
	if (!this.tourists) {
		var options = {
			data: data,
		...
		this.tourists = new NIMUIKit.TeamList(options)
		this.tourists.inject($('#tourists').get(0))
	} else {
		this.tourists.update(data)
	}
	this.doPoint()
}
...
YX.fn.clickTeamAvatar = function (account, type) {
	$("#tourists").find("li.active").removeClass("active")
		...
		this.mysdk.createTeam({
			...
			accounts: accounts,
				...
				if (!error) {
					...
					that.buildTourists()
					$('#touristWrap .j-normalTeam li[data-account="' + t.team.teamId + '"]').click()
		...
		this.mysdk.createTeam({
			...
			joinMode: 'needVerify',
				...
				if (!error) {
					that.cache.addTeam(t.team)
					that.buildTourists()
					$('#touristWrap .j-advanceTeam li[data-account="' + t.team.teamId + '"]').click()
